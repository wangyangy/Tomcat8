...
/**
 * 将XML文件中配置的组件生成java对象,该类中比较重要的方法是startElement,endElement,endDocument等函数
...
public class Digester extends DefaultHandler2 {
    ...
    public void endElement(String namespaceURI, String localName, String qName)
        ...
        //将bodyText中所有"${xxx}"替换成对应的真实属性值，实际也是调用system.getProperty(key)完成的。
        ...
        if ((rules != null) && (rules.size() > 0)) {
            ...
            for (int i = 0; i < rules.size(); i++) {
                try {
                    ...
                    //为得到的rules分别执行rule.body(namespaceURI, name, bodyText);
        ...
        // 为得到的rules分别执行rule.end(namespaceURI, name);
        if (rules != null) {
            for (int i = 0; i < rules.size(); i++) {
        ...
        // 更新match，相当于返回到与之对应startElement之前的match；
    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes list)
        ...
        // 将System中的属性${xxx}设置为真实的属性,通过System.getProperty实现的
    ...
    public void addRuleSet(RuleSet ruleSet) {
        ...
        //先保存一下newNamespaceURI,下面的操作会更改newNamespaceURI
